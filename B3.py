...
PREPROCESSED_FOLDER = '預處理'   

# 圖片副檔名
IMAGE_EXTENSIONS = ['*.jpg', '*.jpeg', '*.png', '*.JPG', '*.JPEG', '*.PNG']

# 模型路徑
MODEL_PATH = r"C:\Users\willes.chen\AA\runs\detect\DB_cell_detection1\weights\best.pt"
...
FILTER_CONFIG = {
    ...
    # YOLO 模型層使用最低的信心度值
    'yolo_conf_threshold': 0.08,  # 原 0.001 → 0.05，先在模型層砍掉極低分框
    # NMS（非極大值抑制）數：過濾重疊的檢測框
    'yolo_iou_threshold': 0.2,   # YOLO 內建 NMS IoU 閾值0.25（越低越積極過濾）
    'same_class_nms_threshold': 0.15,  # 同類別 NMS IoU 閾值0.2（放寬以減少過濾）
    'cross_class_iou_threshold': 0.1,  # 跨類別 NMS IoU 閾值0.5（越高越寬鬆）
    
    # 面積過濾
    'min_area_by_class': {
        'RFID': 0.001,   # RFID 最小面積比例
        'cell': 0.0001,   # cell 最小面積比例
        ...
        while sorted_objects:
            ...
            for kept in keep:
                ...
                if iou > iou_threshold:
                    # 重疊：使用判別式決定保留哪個
                    kept_conf = kept['confidence']
                    
                    # 判別式邏輯：保留信心度較高的框
                    if current_conf > kept_conf:
                        # current 信心度更高，保留 current，丟掉 kept
                        continue  # 不加入 kept 到 new_keep
                    else:
                        # kept 信心度更高或相同，保留 kept，丟掉 current
                        keep_current = False
...
def apply_cross_class_nms(detected_objects, iou_threshold):
    """跨類別 NMS 使用判別式邏輯 優先權高的保留 同時保留信心度較高的"""
    if len(detected_objects) <= 1:
        return detected_objects
    ...
    while sorted_objects:
        # 取出信心度最高的框
        current = sorted_objects.pop(0)
        current_priority = class_priority.get(current['class'], 0)
        current_conf = current['confidence']
        ...
        for kept in keep:
            ...
            if iou > iou_threshold:
                # 重疊：使用判別式決定保留哪個
                kept_priority = class_priority.get(kept['class'], 0)
                kept_conf = kept['confidence']
                
                # 判別式邏輯：
                # 1. 如果 current 優先權更高 → 保留 current，丟掉 kept
                # 2. 如果 kept 優先權更高 → 保留 kept，丟掉 current
                # 3. 如果優先權相同 → 保留信心度更高的
                
                if current_priority > kept_priority:
                    # current 優先權更高，保留 current，丟掉 kept
                    continue  # 不加入 kept 到 new_keep
                elif current_priority < kept_priority:
                    # kept 優先權更高，保留 kept，丟掉 current
                    keep_current = False
                    new_keep.append(kept)
                else:
                    # 優先權相同，保留信心度更高的
